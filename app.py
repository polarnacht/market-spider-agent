...
if "api_key" in st.secrets:
    ...
    API_KEY = "sk-cc6655649d204550bd5bcffd355ab4dd" # 替换为你的真实 Key
...
with st.expander("📌 数据源规格与指令说明 (点击展开)", expanded=True):
    ...
    * **【单点宏观大盘】**：`分析 玩匠4月整体情况` *(自动抓取4月全月开测数据)*
    * **【PC端游大盘】**：`分析目前所有 PC 游戏大盘情况` *(联动 愿望热度 + 畅销流水)*
    * **【全局聚合研报】**：`生成全行业泛娱乐分析简报` *(五源并发)*
    """)
...
def parse_intent(prompt):
    ...
    if any(kw in p_lower for kw in ["全行业", "泛娱乐", "所有市场"]):
        ...
        with st.status(f"🚀 识别到 {len(tasks)} 个采集路由，正在下发调度...", expanded=True) as status:
            for task in tasks:
                ...
                if ret_code == 0 and os.path.exists(res_csv):
                    ...
                    st.success(f"✅ `{module_name}` 抓取完成，成功提取 {data_len} 条数据。")
            ...
            if total_data_count > 0:
                ...
                if len(tasks) >= 3:
                    ...
                    analysis_strategy = "进行赛道的宏观商业数据概览。重点关注：1. 头部资源的集中度；2. 断层领先的标的；3. 赛道主流品类与变现效率。"
...
开始执行：
        ...
        st.warning("指令无法解析。请确保指令包含触发关键字（如：Steam畅销榜 / 玩匠 / PC大盘）。")
